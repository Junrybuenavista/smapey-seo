...
import {
  ...
  Menu, X, Star, Bell, CreditCard, BarChart3, Shield,
  Scissors, CalendarCheck, UserCheck,
} from "lucide-react"

const FEATURES = [
  {
    icon: CalendarCheck,
    title: "Full Appointment Scheduling",
    desc: "Manage your entire salon's appointment book from one dashboard. Book, confirm, reschedule, and complete — all in one place.",
  ...
  },
  {
    ...
    desc: "Set each stylist's working hours per day and assign appointments accordingly. No more manual coordination or whiteboard calendars.",
  ...
  },
  {
    ...
    desc: "Set custom durations per service so your schedule never overlaps. A blowout takes 30 mins; a full color takes 2 hours — handled automatically.",
  ...
  },
  {
    ...
    desc: "Collect deposits at booking time to reduce no-shows. Track payment status without a separate spreadsheet or cash register log.",
    color: "from-emerald-600 to-teal-500",
    shadow: "shadow-emerald-500/20",
  },
  {
    icon: Bell,
    title: "Appointment Status Workflow",
    desc: "Move appointments through Pending → Confirmed → Completed with one click. Track cancellations and no-shows automatically.",
    color: "from-cyan-500 to-teal-400",
    shadow: "shadow-cyan-400/20",
  },
  {
    icon: UserCheck,
    title: "Client-Stylist Assignment",
    desc: "Assign every appointment to a specific stylist. Clients always see who they're booked with, and your team stays coordinated.",
  ...
  },
  {
    ...
    desc: "See today's full appointment schedule, upcoming bookings, stylist workloads, and monthly totals from one live dashboard.",
  ...
  },
  {
    ...
    desc: "Invite your stylists and front desk team with role-based permissions. Control who can book, edit, or only view the schedule.",
  ...
  },
  {
    ...
    desc: "Your client data and appointment records are fully isolated. Nothing is shared across other salons or businesses.",
...
const FAQS = [
  {
    q: "Can I schedule appointments for multiple stylists at the same time?",
    a: "Yes. Each stylist has their own availability schedule. When you book a client, you assign them to a specific stylist — preventing double bookings automatically.",
  },
  {
    q: "How does service duration affect scheduling?",
    a: "Each service has a custom duration you define. The system uses that duration to block off the right amount of time for each appointment, preventing overlap.",
  },
  {
    q: "Can I require a deposit when booking?",
    a: "Yes. You can record a deposit amount when creating an appointment and mark it as paid or unpaid. This helps reduce no-shows significantly.",
  },
  {
    q: "Is this for online booking or internal scheduling?",
    a: "It's an internal scheduling tool — your staff books appointments through the dashboard when clients call or walk in. Ideal for salons that prefer to control their own booking process.",
  },
]
...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0a1a1a]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <a href="/booking" className="text-sm text-white/60 hover:text-white transition-colors">Home</a>
      ...
      {open && (
        <div className="md:hidden bg-[#0a1a1a] border-t border-white/5 px-6 py-4 flex flex-col gap-4">
          <a href="/booking" className="text-sm text-white/60 hover:text-white transition-colors">Home</a>
          <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=BOOKING&plan=FREE`}
            className="text-sm font-semibold px-4 py-2 rounded-lg bg-teal-600 text-white text-center">Get started</a>
        </div>
      )}
    </nav>
  )
...
function Hero() {
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden"
      style={{ background: "#0a1a1a", backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0px, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 28px)", backgroundSize: "28px 28px" }}>
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs font-semibold mb-6">
          ...
          Built for salons & beauty studios
        </div>
        ...
        <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
          Smapey Booking gives your salon a complete appointment scheduling system — manage your full book,
          assign stylists, track deposits, and run your schedule without chaos.
        </p>
    ...
    <section id="features" className="py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">Complete salon appointment scheduling</h2>
          <p className="text-slate-500 mt-4 max-w-xl mx-auto">Everything you need to manage your salon's full schedule — from the first booking to the final checkout.</p>
        </Animate>
    ...
    <section id="pricing" className="py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <p className="text-slate-500 mt-4">Start free. Scale as your salon grows.</p>
          {isPhilippines !== null && (
          ...
          {PLANS.map((p, i) => {
            ...
            return (
              <Animate key={p.name} delay={i * 100}>
                <div className={`rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-1 ${p.highlight ? "bg-gradient-to-b from-teal-700 to-teal-900 border-teal-500/30 shadow-2xl shadow-teal-600/20 scale-105" : "bg-white border-slate-200 shadow-sm hover:shadow-md"}`}>
                  {p.highlight && <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-400/20 text-emerald-300 text-xs font-semibold mb-4"><Star className="w-3 h-3" /> Most popular</span>}
                  ...
                  <button onClick={() => handleSelect(p)}
                    className={`w-full text-center py-2.5 rounded-xl text-sm font-semibold transition-all ${p.highlight ? "bg-emerald-400 hover:bg-emerald-300 text-emerald-900 shadow-lg shadow-emerald-400/25" : "bg-teal-600 hover:bg-teal-500 text-white shadow-md shadow-teal-600/20"}`}>
                    {p.cta}
                  </button>
                </div>
              </Animate>
            ...
            <Animate key={i} delay={i * 60}>
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button onClick={() => setOpen(open === i ? null : i)}
                  ...
                  {q}<ChevronRight className={`w-4 h-4 text-teal-400 transition-transform duration-200 shrink-0 ${open === i ? "rotate-90" : ""}`} />
                </button>
  ...
  return (
    <section className="py-24 relative overflow-hidden"
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-4">Ready to take control of your salon schedule?</h2>
        <p className="text-white/40 mb-8">Join salons that use Smapey Booking to run their appointment scheduling.</p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=BOOKING&plan=FREE`}
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-teal-700 to-teal-500 px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-teal-100 text-sm mt-0.5">{plan.name} plan — <span className="font-semibold">{displayPrice}</span>{plan.period}</p>
            </div>
          </div>
        ...
        <div className="p-6 flex flex-col gap-4">
          {step === "details" && (
            <>
              <input placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400" />
              <input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400" />
              <button onClick={handleContinue} disabled={loading !== null} className="w-full py-3 rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-60">
          ...
          {step === "payment" && (
            <>
              ...
              <button onClick={() => checkout("paypal")} disabled={loading !== null} className="w-full flex items-center gap-4 px-5 py-4 border-2 border-slate-200 rounded-2xl hover:border-teal-500 hover:bg-teal-50 transition-all group">
                ...
                <div className="flex-1 text-left"><p className="text-sm font-semibold text-slate-800 group-hover:text-teal-700">PayPal</p><p className="text-xs text-slate-400">Pay with your PayPal account</p></div>
...
export default function SalonSchedulingContent() {
  return (
    <main>
      <Navbar />
      <Hero />
