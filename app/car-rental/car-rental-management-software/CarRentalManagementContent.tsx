...
import { Car, CheckCircle2, ChevronRight, Menu, X, Star, BarChart3, Wrench, AlertCircle, Calendar, Key, Users } from "lucide-react"
...
const FEATURES = [
  ...
  { icon: Calendar, title: "Rental lifecycle", desc: "Manage the full cycle — Reserved → Active → Returned or Cancelled. Each stage is one click and every transition is logged.", color: "from-orange-500 to-yellow-500", shadow: "shadow-orange-400/20" },
  { icon: AlertCircle, title: "Overdue management", desc: "Rentals past their return date are auto-flagged as Overdue. See them immediately on your dashboard and take action before it becomes a problem.", color: "from-orange-700 to-amber-600", shadow: "shadow-orange-600/20" },
  { icon: BarChart3, title: "Revenue & utilization", desc: "See monthly revenue, rental counts, and fleet utilization side by side. Identify your highest-earning vehicles and your idle ones.", color: "from-amber-500 to-orange-400", shadow: "shadow-amber-400/20" },
  { icon: Key, title: "Upcoming returns", desc: "View all active rentals sorted by return date. Know exactly who's coming back today and this week — no manual calendar tracking needed.", color: "from-orange-600 to-yellow-400", shadow: "shadow-orange-500/20" },
]

const PLANS = [
  { name: "Free", phpPrice: "₱0", usdPrice: "$0", period: "/mo", planKey: "FREE", product: "CAR_RENTAL", desc: "For operators just getting started.", features: ["Up to 5 vehicles", "20 rentals / month", "Customer records", "2 team members"], cta: "Get started free", highlight: false },
  { name: "Pro", phpPrice: "₱599", usdPrice: "$12", period: "/mo", planKey: "PRO", product: "CAR_RENTAL", desc: "For growing rental operations.", features: ["Up to 30 vehicles", "Unlimited rentals", "Overdue tracking", "Revenue dashboard", "5 team members"], cta: "Start Pro", highlight: true },
  { name: "Enterprise", phpPrice: "₱999", usdPrice: "$20", period: "/mo", planKey: "ENTERPRISE", product: "CAR_RENTAL", desc: "For large fleets with no limits.", features: ["Unlimited vehicles", "Unlimited rentals", "Everything in Pro", "Unlimited team members", "Priority support"], cta: "Get Enterprise", highlight: false },
]
...
function PaymentModal({ plan, isPhilippines, onClose }: { plan: typeof PLANS[0] | null; isPhilippines: boolean; onClose: () => void }) {
  ...
  const checkout = async (method: CheckoutMethod) => {
    try {
      setLoading(method); const isLoggedIn = !!token
      ...
      const data = await res.json(); if (!res.ok) throw new Error(data.message); window.location.href = data.approveUrl || data.checkoutUrl
    ...
    <main>
      <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#1a0800]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8"><a href="/car-rental" className="text-sm text-white/60 hover:text-white">Home</a><a href="#features" className="text-sm text-white/60 hover:text-white">Features</a><a href="#pricing" className="text-sm text-white/60 hover:text-white">Pricing</a></div>
      ...
      <section className="relative min-h-screen flex items-center pt-16 overflow-hidden" style={{ background: "#1a0800", backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0px, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 28px)", backgroundSize: "28px 28px" }}>
        <div className="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(245,158,11,0.12) 0%, transparent 70%)" }} />
        <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-semibold mb-6"><Wrench className="w-3 h-3" />Fleet management made simple</div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white leading-tight tracking-tight mb-6">Car rental management software that <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-amber-400 to-yellow-400">runs your whole operation</span></h1>
          <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">Track your fleet status, manage the full rental lifecycle, catch overdue vehicles, and see your revenue — all from one clean dashboard.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <a href="#features" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white/70 font-medium text-sm border border-white/10">See features</a>
          </div>
          <div className="mt-16 flex flex-wrap items-center justify-center gap-6 text-white/30 text-xs">{["No credit card required", "Free plan forever", "Set up in minutes"].map(t => (<span key={t} className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-orange-400" />{t}</span>))}</div>
        </div>
      </section>

      <section id="features" className="py-24 bg-slate-50">
        <div className="max-w-6xl mx-auto px-6">
          <Animate className="text-center mb-16"><p className="text-orange-600 text-sm font-semibold uppercase tracking-widest mb-3">Management Features</p><h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">Full control over your rental operation</h2><p className="text-slate-500 mt-4 max-w-xl mx-auto">Every tool you need to run a rental business — nothing you don't.</p></Animate>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">{FEATURES.map(({ icon: Icon, title, desc, color, shadow }, i) => (<Animate key={title} delay={i * 80}><div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all duration-300 h-full"><div className={`w-11 h-11 rounded-xl bg-gradient-to-tr ${color} shadow-lg ${shadow} flex items-center justify-center mb-4`}><Icon className="w-5 h-5 text-white" /></div><h3 className="font-bold text-slate-800 mb-2">{title}</h3><p className="text-slate-500 text-sm leading-relaxed">{desc}</p></div></Animate>))}</div>
        </div>
      </section>
