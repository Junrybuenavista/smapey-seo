...
const FEATURES = [
  {
    ...
    desc: "Register patients with full profiles — name, contact, date of birth, allergies, and clinical notes. Everything your front desk needs in one place.",
  ...
  },
  {
    ...
    desc: "Add doctors with specialties, contact info, and per-day availability slots. Activate or deactivate doctors without deleting their history.",
  ...
  },
  {
    ...
    desc: "Book appointments by patient, doctor, date, and time. Attach a chief complaint and notes — no paper log, no scattered spreadsheets.",
  ...
  },
  {
    ...
    desc: "A real-time kanban board that shows every patient in Waiting, In Consultation, or Done — for every doctor in your clinic, all at once.",
  ...
  },
  {
    ...
    desc: "Today's appointments, weekly completion trends, in-queue counts, and monthly totals — visualized on a clean dashboard you can read in seconds.",
  ...
  },
  {
    ...
    desc: "Set availability per doctor by day of week and time slot. The system knows who's available and when — reducing double-bookings.",
  ...
  },
  {
    ...
    desc: "Move patients through Pending → Confirmed → In Queue → In Progress → Completed with one click. Track no-shows and cancellations automatically.",
  ...
  },
  {
    ...
    desc: "Invite receptionists, nurses, and clinic admins with role-based permissions. Control exactly who can book, edit, or only view appointments.",
  ...
  },
  {
    ...
    desc: "Each clinic gets its own isolated data space. Your patient list, doctor profiles, and appointment history are never shared with anyone else.",
...
const FAQS = [
  {
    ...
    a: "A clinic management system is software that helps medical practices track patients, manage doctor schedules, book appointments, and run a patient queue — all in one place instead of spreadsheets and paper logs.",
  },
  {
    q: "Can I manage multiple doctors in one clinic?",
    a: "Yes. You can add multiple doctors with individual schedules and specialties. Appointments are assigned to specific doctors, and the queue board shows all active patients per doctor at a glance.",
  },
  {
    q: "How does the live queue board work?",
    a: "The queue board is a real-time kanban view with three columns: Waiting, In Consultation, and Done. You can move patients between columns with one click — Confirm, Enqueue, Start, Complete, or mark No Show.",
  },
  {
    q: "Is this built for a specific clinic type?",
    a: "Smapey Clinic Manager works for general practice clinics, dental clinics, dermatology offices, pediatric clinics, physiotherapy centers, and any medical practice that manages patient appointments.",
  },
  {
    q: "How is patient data protected?",
    a: "Every clinic account is fully isolated — your patient records, appointment history, and doctor data are never visible to other businesses on the platform. Data is encrypted at rest and in transit.",
  },
]
    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#050d1a]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/clinic/guide" className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white transition-colors">
      ...
      {open && (
        <div className="md:hidden bg-[#050d1a] border-t border-white/5 px-6 py-4 flex flex-col gap-4">
          ...
          <Link href="/clinic/guide" onClick={() => setOpen(false)} className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white transition-colors">
            <BookOpen className="w-3.5 h-3.5" /> Guide
          </Link>
          <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=CLINIC&plan=FREE`}
    ...
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden"
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-6">
          ...
          Clinic management system for small &amp; mid-size practices
        </div>
        ...
        <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
          Smapey Clinic Manager handles patients, doctors, appointments, and a live queue board — all in one clean dashboard. Built for clinics that want less chaos, not more software.
        </p>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <p className="text-slate-500 mt-4 max-w-xl mx-auto">
            From patient registration to live queue management — Smapey Clinic Manager keeps your practice running without paper logbooks or scattered tools.
          </p>
        </Animate>
            ...
            return (
              <Animate key={p.name} delay={i * 100}>
                <div className={`rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-1 ${
                  p.highlight
                    ? "bg-gradient-to-b from-blue-800 to-blue-950 border-blue-500/30 shadow-2xl shadow-blue-600/20 scale-105"
    ...
    <section className="py-24 relative overflow-hidden"
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        ...
        <p className="text-white/40 mb-8">
          Join clinics and medical practices using Smapey to manage patients, doctors, and appointments without the chaos.
        </p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=CLINIC&plan=FREE`}
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm mt-0.5">{plan.name} plan — <span className="font-semibold">{displayPrice}</span>{plan.period}</p>
            </div>
          </div>
