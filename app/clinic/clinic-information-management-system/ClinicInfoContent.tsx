...
import {
  ClipboardList, Users, CalendarDays, CheckCircle2, ChevronRight,
  Menu, X, Star, BarChart3, Shield, Database, HeartPulse,
} from "lucide-react"
...
const FEATURES = [
  {
    ...
    desc: "Store complete patient profiles — name, contact, date of birth, allergies, conditions, and clinical notes. Search and retrieve any record in seconds.",
    color: "from-blue-600 to-blue-400", shadow: "shadow-blue-500/20",
  },
  {
    icon: CalendarDays,
    title: "Appointment History",
    desc: "Every appointment — past, present, and future — is stored and linked to the patient. See the full visit history for any patient at any time.",
    color: "from-cyan-600 to-blue-400", shadow: "shadow-cyan-500/20",
  },
  {
    icon: ClipboardList,
    title: "Doctor & Schedule Records",
    desc: "Maintain a complete directory of your doctors — specialties, contact details, availability slots, and the appointments assigned to each one.",
    color: "from-blue-500 to-cyan-400", shadow: "shadow-blue-400/20",
  },
  {
    icon: BarChart3,
    title: "Analytics & Reporting",
    desc: "Turn your clinic's data into insights — daily totals, weekly trends, monthly volumes, completion rates, and cancellation counts — all in real time.",
    color: "from-blue-700 to-blue-500", shadow: "shadow-blue-600/20",
  },
  {
    icon: Database,
    title: "Centralized Data",
    desc: "All clinic information lives in one system — patients, doctors, appointments, and queue history. No more scattered spreadsheets or disconnected tools.",
    color: "from-cyan-500 to-blue-500", shadow: "shadow-cyan-500/20",
  },
  {
    icon: Shield,
    title: "Secure & Isolated Storage",
    desc: "Your clinic's information is encrypted at rest and in transit, stored in a fully isolated account. No sharing, no leakage to other businesses.",
    color: "from-blue-600 to-cyan-500", shadow: "shadow-blue-500/20",
  },
]

const STEPS = [
  { step: "01", title: "Enter clinic data once", desc: "Add your doctors and register patients when they first visit. The system stores everything for future appointments." },
  { step: "02", title: "Build your appointment history", desc: "As appointments are booked and completed, your clinic automatically builds a rich record of every patient interaction." },
  { step: "03", title: "Access anything instantly", desc: "Search patients by name or phone, filter appointments by doctor or date, and pull up analytics anytime." },
]
...
function Hero() {
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden" style={{ background: "#050d1a", backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0px, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 28px)", backgroundSize: "28px 28px" }}>
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-6">
          <HeartPulse className="w-3 h-3" /> Clinic information management system
        </div>
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white leading-tight tracking-tight mb-6">
          All your clinic information{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-300">organized in one place</span>
        </h1>
        <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
          Patients, doctors, appointments, and analytics — stored securely, searchable instantly, and always up to date. No spreadsheets. No paper files.
        </p>
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-6 text-white/30 text-xs">
          {["No credit card required", "Free plan forever", "Set up in under 5 minutes"].map((t) => <span key={t} className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-blue-400" />{t}</span>)}
        </div>
      </div>
    </section>
  )
    ...
    <section id="features" className="py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">Your complete clinic information hub</h2>
          <p className="text-slate-500 mt-4 max-w-xl mx-auto">From patient records to doctor directories to appointment history — all organized, searchable, and secure.</p>
        </Animate>
    ...
    <section className="py-24 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">Build your clinic's information base</h2>
        </Animate>
  ...
  return (
    <section className="py-24 relative overflow-hidden" style={{ background: "#050d1a", backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0px, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 28px)", backgroundSize: "28px 28px" }}>
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-4">Get your clinic's information under control.</h2>
        <p className="text-white/40 mb-8">Free forever for small practices. No setup cost, no contract required.</p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=CLINIC&plan=FREE`} className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-semibold transition-all shadow-xl shadow-blue-600/20">Get started for free <ChevronRight className="w-4 h-4" /></a>
      </Animate>
    </section>
  )
}
...
function PaymentModal({ plan, isPhilippines, onClose }: { plan: { name: string; phpPrice: string; usdPrice: string; period: string; planKey: string; product: string } | null; isPhilippines: boolean; onClose: () => void }) {
  ...
  const checkout = async (method: CheckoutMethod) => {
    try { setLoading(method); const isLoggedIn = !!token; const endpoint = isLoggedIn ? (method === "paypal" ? "/api/billing/subscribe/paypal" : "/api/billing/subscribe/paymongo") : (method === "paypal" ? "/api/billing/newaccount/paypal" : "/api/billing/newaccount/paymongo"); const payload = isLoggedIn ? { product: plan.product, plan: plan.planKey } : { name, email, product: plan.product, plan: plan.planKey }; const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify(payload) }); const data = await res.json(); if (!res.ok) throw new Error(data.message); window.location.href = data.approveUrl || data.checkoutUrl
...
export default function ClinicInfoContent() {
  return (
    <main>
      <Navbar />
      <Hero />
      ...
      <FAQ />
      <CTA />
      <InternalLinks cluster="clinic" currentPath="/clinic/clinic-information-management-system" />
      <Footer />
    </main>
  )
}
