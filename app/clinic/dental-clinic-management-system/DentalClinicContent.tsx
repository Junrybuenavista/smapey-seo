...
import {
  ...
  Menu, X, Star, BarChart3, Shield, Clock, ListOrdered, HeartPulse,
} from "lucide-react"
...
const FEATURES = [
  {
    ...
    desc: "Book patient appointments by dentist, date, and time. Attach procedure notes — cleaning, extraction, filling, root canal — so the right chair is always ready.",
    color: "from-blue-600 to-blue-400",
    shadow: "shadow-blue-500/20",
  },
  {
    icon: Stethoscope,
    title: "Dentist & Staff Management",
    desc: "Add multiple dentists with individual schedules and specialties. Assign each appointment to the right practitioner — general dentist, orthodontist, or oral surgeon.",
  ...
  },
  {
    ...
    desc: "Track every patient in real time — Waiting, In Chair, Done. No more calling out names or checking a paper list. One screen, every status.",
  ...
  },
  {
    ...
    desc: "Keep a patient profile for every regular visitor — contact info, date of birth, notes on conditions or allergies. Pull it up in seconds when they arrive.",
  ...
  },
  {
    ...
    desc: "Today's appointments at a glance — who's in, who's waiting, what's complete. Weekly charts show appointment trends so you can plan ahead.",
  ...
  },
  {
    ...
    desc: "Patient records are fully isolated per dental clinic. Your data is never mixed with other businesses — encrypted at rest and in transit.",
...
const FAQS = [
  {
    q: "Is this specifically built for dental clinics?",
    a: "The system works for any clinic type, including dental. You can name your practitioners as dentists, add procedure types as services, and use the queue board to manage chairs instead of rooms.",
  },
  {
    q: "Can I manage multiple dentists?",
    a: "Yes. Add each dentist with their own schedule slots, specialty, and contact info. Appointments are assigned per dentist so the queue board stays clear.",
  },
  {
    q: "Does it handle appointment reminders?",
    a: "Currently the system is front-desk operated — staff create and manage appointments. The dashboard shows today's schedule so your team is always prepared.",
  },
  {
    q: "Is there a free plan?",
    a: "Yes. The free plan lets you run a small dental practice with core features — patient records, appointment booking, and the live queue board — at no cost.",
  },
]
    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#050d1a]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <img src="/logo.png" alt="Smapey Clinic Manager" className="w-8 h-8 rounded-lg object-cover" />
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <a href="/clinic/guide" className="text-sm text-white/60 hover:text-white transition-colors">Guide</a>
        </div>
      ...
      {open && (
        <div className="md:hidden bg-[#050d1a] border-t border-white/5 px-6 py-4 flex flex-col gap-4">
          ...
          <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=CLINIC&plan=FREE`}
            className="text-sm font-semibold px-4 py-2 rounded-lg bg-blue-600 text-white text-center">Get started</a>
        </div>
      )}
    </nav>
  )
    ...
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden"
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-6">
          <HeartPulse className="w-3 h-3" /> Built for dental clinics
        </div>
        ...
        <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
          Manage dental appointments, track patients, assign dentists, and run a live chair queue — from one simple dashboard. No steep learning curve, no expensive licenses.
        </p>
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-6 text-white/30 text-xs">
          {["No credit card required", "Free plan forever", "Set up in under 5 minutes"].map((t) => (
            <span key={t} className="flex items-center gap-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-blue-400" />{t}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <p className="text-slate-500 mt-4 max-w-xl mx-auto">
            From scheduling a cleaning to managing an orthodontist's full day — Smapey keeps your dental practice organized.
          </p>
        </Animate>
    ...
    <section id="pricing" className="py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <p className="text-slate-500 mt-4">Start free. Scale as your dental practice grows.</p>
          {isPhilippines !== null && (
                  ...
                  <ul className="space-y-3 mb-8">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm">
                        <CheckCircle2 className={`w-4 h-4 shrink-0 ${p.highlight ? "text-cyan-300" : "text-blue-500"}`} />
                        <span className={p.highlight ? "text-blue-100/80" : "text-slate-600"}>{f}</span>
                      </li>
                    ))}
                  </ul>
                  <button onClick={() => handleSelect(p)}
                    className={`w-full text-center py-2.5 rounded-xl text-sm font-semibold transition-all ${p.highlight ? "bg-cyan-400 hover:bg-cyan-300 text-cyan-900 shadow-lg shadow-cyan-400/25" : "bg-blue-600 hover:bg-blue-500 text-white shadow-md shadow-blue-600/20"}`}>
                    {p.cta}
                  </button>
                </div>
              </Animate>
  ...
  return (
    <section className="py-24 relative overflow-hidden"
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-4">Start managing your dental clinic smarter.</h2>
        <p className="text-white/40 mb-8">Free forever for small practices. No setup fee. No contract.</p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=CLINIC&plan=FREE`}
    ...
    <footer className="bg-[#030810] border-t border-white/5 px-6 py-8">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <img src="/logo.png" alt="Smapey Clinic Manager" className="w-6 h-6 rounded-md object-cover" />
          <span className="text-white/60 text-sm font-semibold">Smapey Clinic Manager</span>
        </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-6 py-5 flex items-center justify-between">
          <div><h2 className="text-white font-bold text-lg">{step === "details" ? "Create your account" : "Choose payment method"}</h2>
            <p className="text-blue-100 text-sm mt-0.5">{plan.name} — <span className="font-semibold">{displayPrice}</span>{plan.period}</p></div>
        ...
        <div className="p-6 flex flex-col gap-4">
          {step === "details" && (
            <><input placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400" />
              ...
              <button onClick={handleContinue} disabled={loading !== null} className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-60">{loading ? <><Spinner /> Redirecting…</> : <>Continue <ChevronRight className="w-4 h-4" /></>}</button></>
          )}
          {step === "payment" && (<>
...
export default function DentalClinicContent() {
  return (
    <main>
      <Navbar />
      <Hero />
      ...
      <FAQ />
      <CTA />
      <InternalLinks cluster="clinic" currentPath="/clinic/dental-clinic-management-system" />
      <Footer />
    </main>
  )
}
