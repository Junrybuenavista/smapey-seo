...
import {
  Package, MessageSquare, Hash, Users, CreditCard, BarChart3,
  StickyNote, UserPlus, CheckCircle2, ChevronRight, Menu, X, Zap, XCircle, Star,
} from "lucide-react"
...
const SERVICE_FEATURES = [
  {
    ...
    desc: "Accept walk-in orders, assign service types, and move orders through a clear status pipeline — Received, Washing, Drying, Ready, Released.",
  ...
  },
  {
    ...
    desc: "Customers get an SMS when you accept their order and another when it's ready for pickup. No manual texting — it all happens automatically.",
    color: "from-blue-600 to-sky-400",
    shadow: "shadow-blue-500/20",
  },
  {
    icon: Hash,
    title: "Auto-Generated Ticket Numbers",
    desc: "Every order gets a unique ticket number in YYMMDD-NNN format. Customers use this to check their order status without calling you.",
  ...
  },
  {
    ...
    desc: "Find or create customers by phone number in seconds. Every order they've placed — including amounts and status — is stored in their profile.",
  ...
  },
  {
    ...
    desc: "Record Cash, GCash, Maya, or Bank Transfer per order. Mark orders as paid or partially paid and see outstanding balances at a glance.",
    color: "from-blue-600 to-cyan-400",
    shadow: "shadow-blue-500/20",
  },
  {
    icon: StickyNote,
    title: "Special Order Notes",
    desc: "Add internal notes for customer preferences — delicate fabrics, specific detergents, folding styles. Staff can see notes without contacting the customer.",
...
const COMPARISON = [
  { feature: "Orders / month", free: "50", pro: "Unlimited", enterprise: "Unlimited" },
  { feature: "Service types", free: "1", pro: "5", enterprise: "Unlimited" },
  { feature: "Team members", free: "2", pro: "5", enterprise: "Unlimited" },
  { feature: "SMS notifications", free: "Manual only", pro: "Auto", enterprise: "Auto" },
  { feature: "Order dashboard", free: true, pro: true, enterprise: true },
...
const FAQS = [
  {
    q: "Does the laundry service app work for pickup and delivery?",
    a: "Yes. You can add delivery addresses and notes per order. The status pipeline tracks whether an order is at the shop, in transit, or delivered.",
  },
  {
    q: "Can customers check their order status on their own?",
    a: "Customers are notified by SMS at key stages — when you accept their order and when it's ready. They don't need an account or app.",
  },
  {
    q: "How do I handle orders with multiple service types?",
    a: "You can attach add-ons to any order. For example, a Wash Dry Fold order can include a fabric conditioner add-on at a separate price.",
  },
  {
    q: "Is there a limit to how many orders I can process per day?",
    a: "On the Free plan, you can process up to 50 orders per month. The Pro plan removes this limit entirely.",
  },
]
...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0a1628]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <a href="/laundry" className="text-sm text-white/60 hover:text-white transition-colors">Home</a>
          <a href="/laundry/guide" className="text-sm text-white/60 hover:text-white transition-colors">Guide</a>
        </div>
...
function Hero() {
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden"
      style={{ background: "#0a1628", backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0px, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 28px)", backgroundSize: "28px 28px" }}>
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-400 text-xs font-semibold mb-6">
          ...
          Purpose-built for laundry service businesses
        </div>
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white leading-tight tracking-tight mb-6">
          The laundry service app{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-400">
            small shops actually need
          </span>
        </h1>
        <p className="text-lg text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
          Accept walk-in orders, track each one through washing and drying, auto-notify customers by SMS,
          and collect payment via GCash or cash — all from one simple dashboard built for laundry shops.
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          <a href="/laundry" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white font-medium text-sm transition-all border border-white/10">
            View all features
          </a>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">
            Everything a laundry service needs
          </h2>
          <p className="text-slate-500 mt-4 max-w-xl mx-auto">
            From accepting the first kilogram to releasing the finished order — LaundryOS handles it all.
          </p>
        </Animate>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICE_FEATURES.map(({ icon: Icon, title, desc, color, shadow }, i) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="text-center px-6 py-4 text-sky-700 font-bold bg-sky-50">Pro <span className="text-xs text-sky-400 font-normal">$19/mo</span></th>
          ...
          {plans.map((p, i) => {
            ...
            return (
              <Animate key={p.name} delay={i * 100}>
                <div className={`rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-1 ${p.highlight ? "bg-gradient-to-b from-sky-700 to-sky-900 border-sky-500/30 shadow-2xl shadow-sky-600/20 scale-105" : "bg-white border-slate-200 shadow-sm hover:shadow-md"}`}>
                  ...
                  <ul className="space-y-3 mb-8">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm">
                        <CheckCircle2 className={`w-4 h-4 shrink-0 ${p.highlight ? "text-cyan-300" : "text-sky-500"}`} />
                        <span className={p.highlight ? "text-sky-100/80" : "text-slate-600"}>{f}</span>
                      </li>
                    ))}
                  </ul>
                  <button onClick={() => handleSelect(p)} className={`w-full text-center py-2.5 rounded-xl text-sm font-semibold transition-all ${p.highlight ? "bg-cyan-400 hover:bg-cyan-300 text-cyan-900 shadow-lg shadow-cyan-400/25" : "bg-sky-600 hover:bg-sky-500 text-white shadow-md shadow-sky-600/20"}`}>{p.cta}</button>
                </div>
              </Animate>
            )
          })}
    ...
    <section className="py-24 relative overflow-hidden"
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-4">
          Start managing your laundry service for free
        </h2>
        <p className="text-white/40 mb-8">No card, no trial, no catch. Just a simple laundry service app that actually works.</p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=LAUNDRY&plan=FREE`}
...
export default function LaundryServiceAppContent() {
  return (
    <main>
      <Navbar />
      <Hero />
      ...
      <FAQ />
      <CTA />
      <InternalLinks cluster="laundry" currentPath="/laundry/laundry-service-app" />
      <Footer />
    </main>
  )
}
