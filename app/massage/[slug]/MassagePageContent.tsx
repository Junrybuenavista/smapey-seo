    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-5 p-6">
      ...
      <div className="text-center">
        ...
        <p className="text-slate-400 text-sm mt-2 max-w-xs">
          <strong className="text-slate-600">{data.org.companyName}</strong> will reach out to confirm your session.
        </p>
      </div>
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-slate-400">{data.org.massageTagline || "Online Booking"}</p>
          </div>
      ...
      <div className="max-w-xl mx-auto px-4 py-6 space-y-4">
            ...
            : <div className="divide-y divide-slate-50">
                {data.services.map(s => (
                  <button key={s.id} type="button" onClick={() => setForm(f => ({ ...f, serviceId: s.id }))}
                    className="w-full flex items-center gap-3 px-5 py-3.5 text-left transition hover:bg-emerald-50/50"
          ...
          <div className="rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
            <div className="px-5 py-4 border-b border-slate-100" style={{ background: soft }}>
              <h2 className="font-bold text-slate-700 text-sm flex items-center gap-2">
                ...
                Pick a Therapist <span className="text-[10px] font-normal text-slate-400">(optional)</span>
              </h2>
            </div>
            <div className="p-4">
        ...
        <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
          ...
          <div className="p-5 space-y-4">
            {[
              ...
              <div key={f.key} className="space-y-1">
                ...
                <input value={form[f.key as keyof FormState]} onChange={field(f.key as keyof FormState)}
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-700 placeholder-slate-300 outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 bg-slate-50" />
              </div>
            ))}
            <div className="space-y-1">
              ...
              <input type="datetime-local" value={form.preferredDate} onChange={field("preferredDate")}
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-700 outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 bg-slate-50 [color-scheme:light]" />
            </div>
            {selectedService && (
            ...
            <div className="space-y-1">
              ...
              <textarea value={form.notes} onChange={field("notes")} rows={3}
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-700 placeholder-slate-300 outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 bg-slate-50 resize-none" />
            </div>
          </div>
          ...
          {data.org.massageDepositEnabled && (
            <div className="mx-5 mb-5 rounded-2xl border p-4 space-y-3" style={{ background: `${accent}06`, borderColor: `${accent}20` }}>
              ...
              {data.org.massageDepositAmount && (
                <p className="text-sm text-slate-500">Send <strong style={{ color: accent }}>{sym}{Number(data.org.massageDepositAmount).toLocaleString()}</strong> to confirm your slot.</p>
              )}
          ...
          <div className="px-5 pb-5 space-y-3">
            ...
            <p className="text-center text-[11px] text-slate-400">Confirmed by {data.org.companyName || "the spa"} after review</p>
          </div>
        </form>
      </div>
    </div>
