...
const FEATURES = [
  { icon: Calendar, title: "Appointment Scheduling", desc: "Book appointments, assign services, and set durations. View your day at a glance in a clean calendar-style list." },
  { icon: Users, title: "Client Management", desc: "Full client profiles with contact info, appointment history, and notes. Find any client by name or phone in seconds." },
    ...
    <nav className="fixed top-0 inset-x-0 z-50" style={{ background: CREAM, borderBottom: `2px solid ${INK}`, fontFamily: display.fontFamily }}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <a href="/salon" className="flex items-center gap-2.5">
          ...
          <span className="font-extrabold tracking-tight" style={{ color: INK }}>Smapey SalonOS</span>
        </a>
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((l) => l === "Guide" ? (
            <a key={l} href="/salon/guide" className="text-sm font-semibold hover:opacity-60 transition-opacity" style={{ color: INK }}>{l}</a>
          ) : (
            <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} onClick={(e) => scrollTo(e, l.toLowerCase().replace(/\s+/g, "-"))} className="text-sm font-semibold hover:opacity-60 transition-opacity" style={{ color: INK }}>{l}</a>
          ))}
        </div>
        <div className="hidden md:flex items-center gap-3">
          ...
          <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=SALON&plan=FREE`} className="text-sm font-bold px-5 py-2.5 rounded-full border-2 transition-transform hover:-translate-y-0.5" style={{ ...display, background: AMBER, color: INK, borderColor: INK, boxShadow: `3px 3px 0 ${INK}` }}>Get started</a>
        </div>
      ...
      {open && (
        <div className="md:hidden px-6 py-4 flex flex-col gap-4" style={{ background: CREAM, borderTop: `2px solid ${INK}` }}>
          {NAV_LINKS.map((l) => l === "Guide" ? (
            <a key={l} href="/salon/guide" className="text-sm font-semibold" style={{ color: INK }}>{l}</a>
          ) : (
            <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} onClick={(e) => scrollTo(e, l.toLowerCase().replace(/\s+/g, "-"))} className="text-sm font-semibold" style={{ color: INK }}>{l}</a>
          ))}
          <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=SALON&plan=FREE`} className="text-sm font-bold px-4 py-2.5 rounded-full border-2 text-center" style={{ ...display, background: AMBER, color: INK, borderColor: INK }}>Get started</a>
        </div>
      )}
    </nav>
  )
          ...
          {STEPS.map(({ step, title, desc }, i) => (
            <Animate key={step} delay={i * 120}>
              <div className="rounded-[22px] border-2 p-8 h-full transition-transform hover:-translate-y-1" style={{ background: "#fff", borderColor: INK, boxShadow: `6px 6px 0 ${acc[i]}` }}>
                <div className="inline-flex items-center justify-center w-14 h-14 rounded-[16px] border-2 font-extrabold text-lg mb-5" style={{ background: acc[i], color: acc[i] === AMBER ? INK : "#fff", borderColor: INK }}>{step}</div>
                <h3 className="font-extrabold mb-2" style={{ color: INK }}>{title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: "#54514c" }}>{desc}</p>
              </div>
            </Animate>
    ...
    <section className="py-24" style={{ background: "#fff", fontFamily: display.fontFamily }}>
      <div className="max-w-4xl mx-auto px-6">
        <Animate className="text-center mb-16">
          ...
          <p className="mt-4" style={{ color: "#54514c" }}>Start free and upgrade only when your salon grows.</p>
        </Animate>
        <Animate>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {limitRows.map(({ feature, key }, i) => (
                  <tr key={feature} style={{ borderBottom: "1px solid rgba(22,22,22,.08)", background: i % 2 === 0 ? "#fff" : "#faf7f1" }}>
                    <td className="px-6 py-4 font-bold" style={{ color: INK }}>{feature}</td>
                    <td className="text-center px-6 py-4"><span className="font-bold" style={{ color: INK }}>{limitDisplay(freePlan, key)}</span></td>
                    <td className="text-center px-6 py-4" style={{ background: "rgba(47,107,255,.07)" }}><span className="font-bold" style={{ color: INK }}>{limitDisplay(proPlan, key)}</span></td>
                    <td className="text-center px-6 py-4"><span className="font-bold" style={{ color: INK }}>{limitDisplay(entPlan, key)}</span></td>
                  </tr>
                ))}
                {FEATURE_ROWS.map((row, i) => (
                  <tr key={row.feature} style={{ borderBottom: "1px solid rgba(22,22,22,.08)", background: (limitRows.length + i) % 2 === 0 ? "#fff" : "#faf7f1" }}>
                    ...
                    {[row.free, row.pro, row.enterprise].map((val, j) => (
                      <td key={j} className="text-center px-6 py-4" style={j === 1 ? { background: "rgba(47,107,255,.07)" } : undefined}>
                        {val ? <CheckCircle2 className="w-5 h-5 mx-auto" style={{ color: BLUE }} /> : <XCircle className="w-5 h-5 mx-auto" style={{ color: "#d8d1c4" }} />}
                      </td>
                    ))}
                  </tr>
                ))}
  ...
  return (
    <section className="py-24 px-6" style={{ background: CREAM, fontFamily: display.fontFamily }}>
      <Animate className="relative max-w-3xl mx-auto rounded-[30px] border-2 p-12 md:p-16 text-center" style={{ background: AMBER, borderColor: INK, boxShadow: `10px 10px 0 ${INK}` }}>
        <h2 className="text-3xl sm:text-5xl font-extrabold tracking-tight mb-4" style={{ color: INK }}>Ready to run your salon smarter?</h2>
        <p className="mb-8 font-medium" style={{ color: "#5c4a28" }}>Join salon owners who use SalonOS to manage appointments and keep clients coming back.</p>
        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=SALON&plan=FREE`} className="inline-flex items-center gap-2 px-8 py-4 rounded-full font-bold border-2 transition-transform hover:-translate-y-0.5" style={{ ...display, background: INK, color: "#fff", borderColor: INK }}>
