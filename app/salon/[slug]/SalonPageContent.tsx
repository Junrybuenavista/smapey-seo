...
import { Scissors, Calendar, User, Phone, Mail, FileText, CheckCircle2, Loader2, AlertCircle, Clock, Sparkles, Star, Zap } from "lucide-react"
...
function BlushPage({ data, form, setForm, handleSubmit, submitting, submitted, setSubmitted, error, sym }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Cover */}
      {data.org.salonCoverUrl && (
        <div className="w-full h-44 overflow-hidden relative">
          <img src={data.org.salonCoverUrl} alt="cover" className="w-full h-full object-cover" />
...
function RosePage({ data, form, setForm, handleSubmit, submitting, submitted, setSubmitted, error, sym }: PageProps) {
  const field = (k: keyof FormState) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm(f => ({ ...f, [k]: e.target.value }))
  const accent = data.org.salonAccentColor || "#f43f5e"
  const selectedService = data.services.find(s => s.id === form.serviceId)
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-5 p-6">
      ...
      <div className="text-center">
        ...
        <p className="text-slate-500 text-sm mt-2 max-w-xs">
          <strong className="text-slate-700">{data.org.companyName}</strong> will get back to you to confirm.
        </p>
      </div>
      <button onClick={() => { setSubmitted(false); setForm(EMPTY_FORM) }}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="relative overflow-hidden" style={{ background: accent }}>
        {data.org.salonCoverUrl && (
          <img src={data.org.salonCoverUrl} alt="cover" className="absolute inset-0 w-full h-full object-cover mix-blend-multiply opacity-30" />
        )}
        <div className="relative max-w-xl mx-auto px-5 py-8">
          ...
          <h1 className="text-4xl font-black text-white uppercase leading-none tracking-tighter">
            {data.org.salonTagline || "Book Your Appointment"}
          </h1>
      ...
      <div className="max-w-xl mx-auto px-4 py-6 space-y-5">
                  ...
                  <button key={s.id} type="button" onClick={() => setForm(f => ({ ...f, serviceId: s.id }))}
                    ...
                    {s.imageUrl
                      ...
                      : <div className="w-full h-20 flex items-center justify-center" style={{ background: `${accent}10` }}>
                          <Scissors className="w-7 h-7" style={{ color: `${accent}60` }} />
                        </div>}
                    <div className="p-3">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="mx-5 mb-5 rounded-xl border p-4 space-y-3 bg-slate-50" style={{ borderColor: `${accent}30` }}>
              ...
              {data.org.salonDepositAmount && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold" style={{ color: accent }}>{sym}{Number(data.org.salonDepositAmount).toLocaleString()}</span>
                </div>
              )}
              {data.org.salonDepositQrUrl && (
                <div className="flex flex-col items-center gap-2">
                  <img src={data.org.salonDepositQrUrl} alt="QR" className="w-40 h-40 rounded-lg bg-white p-1 border border-slate-200 object-contain" />
                  {data.org.salonDepositNote && <p className="text-xs text-slate-400 text-center">{data.org.salonDepositNote}</p>}
                </div>
              )}
              <input value={form.depositReference} onChange={e => setForm(f => ({ ...f, depositReference: e.target.value }))}
          ...
          <div className="px-5 pb-5 space-y-3">
            ...
            <p className="text-center text-xs text-slate-400">Confirmed by {data.org.companyName || "the salon"} after review</p>
          </div>
        </form>
      </div>
    </div>
...
function SlatePage({ data, form, setForm, handleSubmit, submitting, submitted, setSubmitted, error, sym }: PageProps) {
  const field = (k: keyof FormState) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm(f => ({ ...f, [k]: e.target.value }))
  const accent = data.org.salonAccentColor || "#6366f1"
  const selectedService = data.services.find(s => s.id === form.serviceId)

  if (submitted) return (
    <div className="min-h-screen bg-indigo-50 flex flex-col items-center justify-center gap-5 p-6">
      ...
      <div className="text-center">
        ...
        <p className="text-slate-500 text-sm mt-2 max-w-xs">
          <strong className="text-slate-700">{data.org.companyName}</strong> will contact you to confirm your appointment.
        </p>
      </div>
      <button onClick={() => { setSubmitted(false); setForm(EMPTY_FORM) }}
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-5 py-3 flex items-center gap-3">
          {data.org.logoUrl
            ...
            : <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold" style={{ background: accent }}>
                <Scissors className="w-4 h-4" />
              </div>}
          <div className="flex-1">
            <p className="font-bold text-slate-800 text-sm">{data.org.companyName || "Salon"}</p>
            {data.org.salonTagline && <p className="text-xs text-slate-400">{data.org.salonTagline}</p>}
          </div>
        ...
        <div className="w-full h-40 overflow-hidden relative">
          ...
          <div className="absolute inset-0 flex items-end max-w-2xl mx-auto px-5 pb-4">
            <div>
              ...
              {data.org.salonTagline && <p className="text-white/80 text-xs">{data.org.salonTagline}</p>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-2xl mx-auto px-5 py-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="p-5 space-y-4">
                {data.org.salonDepositAmount && (
                  <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-slate-50 border border-slate-200">
                    ...
                    <span className="font-bold text-lg" style={{ color: accent }}>{sym}{Number(data.org.salonDepositAmount).toLocaleString()}</span>
                  </div>
                )}
                {data.org.salonDepositQrUrl && (
                  <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-center text-xs text-slate-400 pb-4">Confirmed by {data.org.companyName || "the salon"} after review</p>
        </form>
      </div>
    </div>
  )
...
function NoirPage({ data, form, setForm, handleSubmit, submitting, submitted, setSubmitted, error, sym }: PageProps) {
  const field = (k: keyof FormState) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm(f => ({ ...f, [k]: e.target.value }))
  const accent = data.org.salonAccentColor || "#7c3aed"
  const cream = "#faf9f7"
  const selectedService = data.services.find(s => s.id === form.serviceId)

  const inp = "w-full px-4 py-3 text-sm text-slate-700 placeholder-slate-300 outline-none border-b border-slate-200 bg-transparent transition"

  if (submitted) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 p-6" style={{ background: cream }}>
      <Star className="w-10 h-10" style={{ color: accent }} />
      <div className="text-center space-y-2">
        <p className="text-2xl font-light tracking-widest uppercase text-slate-700">Request Sent</p>
        <p className="text-slate-400 text-sm max-w-xs">
          <em>{data.org.companyName}</em> will contact you shortly to confirm.
        </p>
      </div>
      <button onClick={() => { setSubmitted(false); setForm(EMPTY_FORM) }}
        className="px-8 py-3 text-sm font-medium border transition hover:text-white"
  ...
  return (
    <div className="min-h-screen" style={{ background: cream }}>
      {/* Elegant header */}
      <div className="border-b sticky top-0 z-10" style={{ background: cream, borderColor: "#e5e0d8" }}>
        <div className="max-w-lg mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {data.org.logoUrl
              ...
              : <Scissors className="w-5 h-5" style={{ color: accent }} />}
            <div>
              <p className="text-sm font-medium tracking-wide text-slate-700">{data.org.companyName || "Salon"}</p>
              {data.org.salonTagline && <p className="text-[10px] text-slate-400 tracking-wider">{data.org.salonTagline}</p>}
            </div>
          </div>
          <p className="text-[9px] uppercase tracking-[0.3em] text-slate-400">Appointments</p>
        </div>
      </div>
      ...
      {data.org.salonCoverUrl && (
        <div className="relative w-full h-40 overflow-hidden">
          <img src={data.org.salonCoverUrl} alt="cover" className="w-full h-full object-cover" />
          <div className="absolute inset-0" style={{ background: `linear-gradient(to bottom, transparent, ${cream})` }} />
          <div className="absolute bottom-4 left-6">
            <p className="font-light tracking-[0.2em] uppercase text-lg text-slate-700">{data.org.companyName}</p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-lg mx-auto px-6 py-8 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-4 mb-5">
            <p className="text-[10px] uppercase tracking-[0.4em] text-slate-400">Services</p>
            <div className="flex-1 h-px bg-slate-200" />
          </div>
          {data.services.length === 0
            ? <p className="text-slate-400 text-sm italic">No services available.</p>
            : <div className="space-y-2">
                {data.services.map(s => (
                  <button key={s.id} type="button" onClick={() => setForm(f => ({ ...f, serviceId: s.id }))}
                    className="w-full text-left flex items-center gap-4 p-4 rounded-xl border transition bg-white"
                    style={form.serviceId === s.id
                      ? { borderColor: accent, borderLeftWidth: 4, paddingLeft: "12px" }
                      : { borderColor: "#e5e0d8" }}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-400 mt-1 flex items-center gap-1"><Clock className="w-3 h-3" />{s.durationMinutes} min</p>
                    </div>
                    <div className="text-right shrink-0">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <div className="flex items-center gap-4 mb-5">
              <p className="text-[10px] uppercase tracking-[0.4em] text-slate-400">Your Details</p>
              <div className="flex-1 h-px bg-slate-200" />
            </div>
            <div className="bg-white rounded-xl border p-5 space-y-0" style={{ borderColor: "#e5e0d8" }}>
              {[
                ...
                <div key={f.key} className={i > 0 ? "border-t border-slate-100" : ""}>
                  <label className="block text-[10px] uppercase tracking-[0.25em] text-slate-400 pt-3">{f.label}</label>
                  <input value={form[f.key as keyof FormState]} onChange={field(f.key as keyof FormState)}
              ...
              <div className="border-t border-slate-100">
                <label className="block text-[10px] uppercase tracking-[0.25em] text-slate-400 pt-3">Date & Time *</label>
                <input type="datetime-local" value={form.preferredDate} onChange={field("preferredDate")}
                  min={dayjs().format("YYYY-MM-DDTHH:mm")} className={inp} style={{ colorScheme: "light" } as any}
                  onFocus={e => e.currentTarget.style.borderColor = accent}
                  onBlur={e => e.currentTarget.style.borderColor = "#e2e8f0"} />
              </div>
              <div className="border-t border-slate-100">
                <label className="block text-[10px] uppercase tracking-[0.25em] text-slate-400 pt-3">Notes</label>
          ...
          {selectedService && (
            <div className="rounded-xl overflow-hidden border" style={{ borderColor: `${accent}30` }}>
              <div className="px-5 py-3 flex items-center justify-between" style={{ background: `${accent}08` }}>
                <div>
          ...
          {data.org.salonDepositEnabled && (
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <p className="text-[10px] uppercase tracking-[0.4em] text-slate-400">Deposit</p>
                <div className="flex-1 h-px bg-slate-200" />
              </div>
              <div className="bg-white rounded-xl border p-5 space-y-4" style={{ borderColor: "#e5e0d8" }}>
                {data.org.salonDepositAmount && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">Amount to pay</span>
                    <span className="font-medium text-lg" style={{ color: accent }}>{sym}{Number(data.org.salonDepositAmount).toLocaleString()}</span>
                  </div>
                )}
                {data.org.salonDepositQrUrl && (
                  <div className="flex flex-col items-center gap-2">
                    <img src={data.org.salonDepositQrUrl} alt="QR" className="w-40 h-40 rounded-lg border border-slate-200 bg-white p-1 object-contain" />
                    {data.org.salonDepositNote && <p className="text-xs text-slate-400 text-center">{data.org.salonDepositNote}</p>}
                  </div>
                )}
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.25em] text-slate-400 mb-2">Reference Number *</label>
                  <input value={form.depositReference} onChange={e => setForm(f => ({ ...f, depositReference: e.target.value }))}
                    placeholder="Transaction reference"
                    className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm text-slate-700 placeholder-slate-300 outline-none bg-slate-50" />
                </div>
              </div>
            </div>
          )}
          ...
          <button type="submit" disabled={submitting}
            className="w-full py-4 rounded-xl font-medium tracking-wide text-sm transition disabled:opacity-50 flex items-center justify-center gap-2 text-white hover:opacity-90"
            style={{ background: accent }}>
            {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : "Send Booking Request"}
          </button>
          <p className="text-center text-xs text-slate-400 pb-4">Confirmed by {data.org.companyName || "the salon"} after review</p>
...
function GoldPage({ data, form, setForm, handleSubmit, submitting, submitted, setSubmitted, error, sym }: PageProps) {
  const field = (k: keyof FormState) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm(f => ({ ...f, [k]: e.target.value }))
  const accent = data.org.salonAccentColor || "#d97706"
  const warm = "#fffbf5"
  const selectedService = data.services.find(s => s.id === form.serviceId)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center gap-5 p-6" style={{ background: warm }}>
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-500 text-sm max-w-xs">
          Thank you! <strong className="text-slate-700">{data.org.companyName}</strong> will confirm your appointment shortly.
        </p>
      </div>
      <button onClick={() => { setSubmitted(false); setForm(EMPTY_FORM) }}
    ...
    <div className="min-h-screen" style={{ background: warm }}>
      ...
      <div className="sticky top-0 z-10 shadow-sm" style={{ background: accent }}>
        <div className="max-w-xl mx-auto px-5 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="font-bold text-white text-sm">{data.org.companyName || "Salon"}</p>
            <p className="text-white/70 text-[11px]">{data.org.salonTagline || "Premium Appointments"}</p>
          </div>
          <div className="flex items-center gap-1">
      ...
      <div className="max-w-xl mx-auto px-4 py-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl border-2 shadow-sm overflow-hidden" style={{ borderColor: "#f3ece0" }}>
          <div className="p-5 space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-500">Notes (optional)</label>
              <textarea value={form.notes} onChange={field("notes")} rows={3} placeholder="Special requests or preferences…"
...
export default function SalonPageContent({ slug }: { slug: string }) {
  const [data, setData]             = useState<{ org: any; services: any[] } | null>(null)
  ...
  useEffect(() => {
    fetch(`${API}/api/salon/public/${slug}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data?.org?.salonDepositEnabled && !form.depositReference) missing.push("deposit reference number")
    if (missing.length > 0) {
      setError(`Please provide: ${missing.join(", ")}.`); return
    }
    setSubmitting(true)
    try {
      const res = await fetch(`${API}/api/salon/public/${slug}/inquire`, {
        method: "POST",
  ...
  const theme = data.org?.salonTheme || "blush"

  if (theme === "rose")  return <RosePage  {...props} />
  if (theme === "slate") return <SlatePage {...props} />
  if (theme === "noir")  return <NoirPage  {...props} />
  if (theme === "gold")  return <GoldPage  {...props} />
  return <BlushPage {...props} />
}
