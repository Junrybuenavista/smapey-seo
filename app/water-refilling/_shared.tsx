...
import { CheckCircle2, ChevronRight, Menu, X, Zap, Droplets } from "lucide-react"
import { usePricing, type Plan } from "@/lib/usePricing"

//////////////////////////////////////////////////////
// CONFIG
//////////////////////////////////////////////////////
export const REGISTER_URL = `${process.env.NEXT_PUBLIC_FRONTEND_URL}/register?product=WATER_REFILLING&plan=FREE`
...
export function Navbar() {
  ...
  const linkHref = (l: string) =>
    l === "Guide" ? "/water-refilling/guide" : `/water-refilling#${l.toLowerCase().replace(/\s+/g, "-")}`

  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      scrolled ? "bg-[#07182a]/90 backdrop-blur-md border-b border-white/5 shadow-lg" : "bg-transparent"
    }`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <a href="/water-refilling" className="flex items-center gap-2.5">
          <img src="/logo.png" alt="Smapey Water Refilling Manager" className="w-8 h-8 rounded-lg object-cover" />
          <span className="text-white font-bold tracking-tight">Smapey Water</span>
        </a>

        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((l) => (
            <a key={l} href={linkHref(l)} className="text-sm text-white/60 hover:text-white transition-colors">
              {l}
            </a>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a href={LOGIN_URL} className="text-sm text-white/60 hover:text-white transition-colors px-4 py-2">
            Sign in
          </a>
          <a
            href={REGISTER_URL}
            className="text-sm font-semibold px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white transition-colors shadow-lg shadow-cyan-600/25"
          >
            Get started
          </a>
        </div>
      ...
      {open && (
        <div className="md:hidden bg-[#07182a] border-t border-white/5 px-6 py-4 flex flex-col gap-4">
          {NAV_LINKS.map((l) => (
            <a key={l} href={linkHref(l)} className="text-sm text-white/60 hover:text-white transition-colors">
              {l}
            </a>
          ))}
          <a href={REGISTER_URL} className="text-sm font-semibold px-4 py-2 rounded-lg bg-cyan-600 text-white text-center">
            Get started
          </a>
        </div>
      )}
    ...
    <section id="pricing" className="py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <Animate className="text-center mb-16">
          <p className="text-cyan-600 text-sm font-semibold uppercase tracking-widest mb-3">Pricing</p>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">
            Simple, transparent pricing
          </h2>
          <p className="text-slate-500 mt-4">Start free. Upgrade when your station grows.</p>
            ...
            return (
              <Animate key={p.name} delay={i * 100}>
                <div className={`rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-1 ${
                  p.highlight
                    ? "bg-gradient-to-b from-cyan-700 to-blue-900 border-cyan-500/30 shadow-2xl shadow-cyan-600/20 scale-105"
                  ...
                  <p className={`text-sm mb-4 ${p.highlight ? "text-cyan-200/60" : "text-slate-400"}`}>{p.desc}</p>
                  <div className="flex items-end gap-1 mb-6">
                    <span className={`text-4xl font-extrabold tracking-tight ${p.highlight ? "text-white" : "text-slate-800"}`}>
                      {displayPrice}
                    </span>
                    <span className={`text-sm mb-1 ${p.highlight ? "text-cyan-200/50" : "text-slate-400"}`}>{p.period}</span>
                  </div>

                  <ul className="space-y-3 mb-8">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm">
                        <CheckCircle2 className={`w-4 h-4 shrink-0 ${p.highlight ? "text-cyan-400" : "text-cyan-500"}`} />
                        <span className={p.highlight ? "text-cyan-100/80" : "text-slate-600"}>{f}</span>
                      </li>
                    ))}
                  </ul>
...
//////////////////////////////////////////////////////
export function CTA({
  title = "Ready to run your water station smarter?",
  subtitle = "Join water station owners who use Smapey to track deliveries, containers, and collections — without the notebook.",
  ...
  return (
    ...
    >
      ...
      <Animate className="relative max-w-2xl mx-auto px-6 text-center">
        ...
        <a
          href={REGISTER_URL}
          className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-400 hover:from-cyan-400 hover:to-blue-300 text-white font-semibold transition-all shadow-xl shadow-cyan-600/20"
        >
...
export function Footer() {
  return (
    <footer className="bg-[#050d18] border-t border-white/5 px-6 py-8">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <img src="/logo.png" alt="Smapey Water" className="w-6 h-6 rounded-md object-cover" />
          <span className="text-white/60 text-sm font-semibold">Smapey Water Refilling Manager</span>
        </div>
        <div className="flex items-center gap-5 text-xs">
          <a href="/water-refilling" className="text-white/30 hover:text-white/60 transition-colors">Overview</a>
          <a href="/water-refilling/guide" className="text-white/30 hover:text-white/60 transition-colors">Guide</a>
...
//////////////////////////////////////////////////////
// SHARED SECTION HELPERS (used across cluster pages)
//////////////////////////////////////////////////////
export function Eyebrow({ children }: { children: React.ReactNode }) {
  return <p className="text-cyan-600 text-sm font-semibold uppercase tracking-widest mb-3">{children}</p>
}
...
// Deep-navy hero shell with water grid + glows, reused by every cluster page.
...
//////////////////////////////////////////////////////
// ARTICLE PRIMITIVES (used by the business/SEO pages)
//////////////////////////////////////////////////////
...
// Soft product pitch band reused at the bottom of the business pages.
export function SoftwarePitch() {
  return (
    <div className="my-12 rounded-3xl border border-cyan-200 bg-gradient-to-br from-cyan-50 to-blue-50 p-8">
      <div className="flex items-start gap-4">
        <span className="w-12 h-12 rounded-2xl bg-cyan-600 flex items-center justify-center shrink-0">
          <Droplets className="w-6 h-6 text-white" />
        </span>
        <div>
          <h3 className="text-xl font-extrabold text-slate-800 mb-2">Run the day-to-day with Smapey Water</h3>
          <p className="text-sm text-slate-600 leading-relaxed mb-4">
            A business plan gets you started — software keeps you running. Smapey tracks every delivery, who's holding your
            containers, your stock and empties, and your daily collections, so you never lose a bottle or a peso.
          </p>
          <a href={REGISTER_URL} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-sm transition-colors shadow-lg shadow-cyan-600/25">
...
export function PaymentModal({
  ...
  const checkout = async (method: CheckoutMethod) => {
    try {
      ...
      if (token) {
        endpoint = method === "paypal" ? "/api/billing/subscribe/paypal" : "/api/billing/subscribe/paymongo"
        payload = { product: plan.product, plan: plan.planKey }
      } else {
        endpoint = method === "paypal" ? "/api/billing/newaccount/paypal" : "/api/billing/newaccount/paymongo"
  ...
  const handleContinue = () => {
    ...
    if (!isPhilippines) checkout("paypal")
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-cyan-700 to-cyan-500 px-6 py-5 flex items-center justify-between">
          ...
          {step === "details" && (
            <>
              <input
                ...
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-400"
              />
              <input
                type="email"
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-60"
              >
